<?xml version="1.0" encoding="UTF-8"?>
<html xsl:version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
<body style="font-family:Arial; font-size:12pt; background-color:teal;">
<h1 style="font-weight:bold; font-family:Arial; font-size:24pt; color:white; text-align: center; padding: 20px; border-bottom: 1px solid #fff;">Students</h1>

<xsl:for-each select="CHRIST/CS/STUDENT">

  <div style="padding: 20px; margin-bottom: 0.5rem; line-height: 1.5">
    
    <div style="color: white; font-weight: bold; font-size: 1.5rem; padding-bottom: 10px;"> 
      <xsl:value-of select="STU-NAME"/>
    </div>

    <div style="color: #ccc; font-weight: 400; font-size: 1rem; padding-bottom: 10px;"> 
      College: 
      <xsl:value-of select="STU-COLLEGE"/>
    </div>
    <div style="color: #ccc; font-weight: 400; font-size: 1rem; padding-bottom: 10px;"> 
      Branch: 
      <xsl:value-of select="STU-BRANCH"/>
    </div>

    <div style="color: #ccc; font-weight: 400; font-size: 1rem; padding-bottom: 10px;"> 
      <xsl:value-of select="STU-EMAIL"/>
    </div>

  </div>

</xsl:for-each>
</body>
</html>